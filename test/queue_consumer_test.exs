defmodule Verk.QueueConsumerTest do
  ...
  describe "init/1" do
    test "sets up redis connection" do
      ...
      assert init([@queue, workers_manager]) == {:ok, state}
    end
  end
  ...
  describe "handle_info/2 consume last_id '>'" do
    test "nil response" do
      ...
      expect(Queue, :consume, fn @queue, @node_id, ">", ^demand, 30_000, :redis ->
        {:ok, nil}
      end)
  ...
  describe "handle_info/2 consume last_id is not '>'" do
    test "nil response" do
      ...
      expect(Queue, :consume, fn @queue, @node_id, ^last_id, ^demand, 30_000, :redis ->
        {:ok, nil}
      end)
